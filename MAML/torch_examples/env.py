...
class Reacher:
    def __init__(self, screen_size=1000, link_lengths = [200, 140, 100], joint_angles=[0, 0, 0], target_pos = [369,430], render=False):
        # Global variables
    ...
    def step(self,action):    
        ...
        if self.steps > self.max_episode_steps:
            self.steps=0
            return np.array([np.concatenate((pos_set,self.target_pos))]), reward, np.array([True])
        else:
            return np.array([np.concatenate((pos_set,self.target_pos))]), reward, np.array([False])
