'''
here’s a minimal example of few-shot regression, predicting a random sine wave 
from 10 (x,y) pairs. This one uses PyTorch and fits in a gist.
'''
import numpy as np
import torch
...
y_bar=np.zeros(np.shape(f_plot(x_all)))
for iteration in range(niterations):
    ...
    f = gen_task()
    y_all = f(x_all)
    '''average the labeled y'''
    y_bar = np.array(iteration*np.array(y_bar)+np.array(y_all))/(iteration+1)
#     print(y_bar)
    # print(y_all)
    # Do SGD on this task
for iteration in range(niterations):
    ...
    for _ in range(innerepochs):
        for start in range(0, len(x_all), ntrain):
            ...
            train_on_batch(x_all[mbinds], y_bar[mbinds])
