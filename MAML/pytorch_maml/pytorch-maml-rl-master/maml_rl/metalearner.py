...
class MetaLearner(object):
    ...
    def adapt(self, episodes, first_order=False):
        ...
        # does not actually change parameters of networm, only calculate new parameters
    ...
    def sample(self, tasks, first_order=False):
        ...
        for task in tasks:
            ...
            print('Start sample one task!')
            train_episodes = self.sampler.sample(self.policy,
                gamma=self.gamma, device=self.device)
            print('Finished sample one task, start update param!')
            params = self.adapt(train_episodes, first_order=first_order)
            print('Finished update param, start sample one task again!')
            # only use the params to sample, not changing network weights actually
            valid_episodes = self.sampler.sample(self.policy, params=params,
                gamma=self.gamma, device=self.device)
            print('Finished sample the task again!')
    ...
    def step(self, episodes, max_kl=1e-3, cg_iters=10, cg_damping=1e-2,
        ...
        shs = 0.5 * torch.dot(stepdir, hessian_vector_product(stepdir)) # dot of 3 matrices, sT.H.s
