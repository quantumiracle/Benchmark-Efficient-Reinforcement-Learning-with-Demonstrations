...
class PPO(object):
    ...
    def sum_gradients_update(self, s, a, r, stepsize):
        ...
        if METHOD['name'] == 'kl_pen':
            ...
            self.tfa = a
            self.tfadv = adv
            a_grads = dict(zip(self.actor_var.keys(), tf.gradients(self.atrain_op, list(self.actor_var.values()))))
            new_a_weights = dict(zip(self.actor_var.keys(), [self.actor_var[key] - stepsize * a_grads[key] 
            for key in self.actor_var.keys()]))
        ...
        self.tfs = s
        self.tfdc_r = r
        c_grads = dict(zip(self.critic_var.keys(), tf.gradients(self.ctrain_op, list(self.critic_var.values()))))
        new_c_weights = dict(zip(self.critic_var.keys(), [self.critic_var[key] - stepsize * c_grads[key] 
        for key in self.critic_var.keys()]))
        ...
        for ep in range(EP_MAX):
            ...
            for t in range(EP_LEN):    #  steps in one episode
                ...
                if (t+1) % BATCH == 0 or t == EP_LEN-1:
                    ...
                    buffer_s, buffer_a, buffer_r = [], [], []
                    # ppo.update(bs, ba, br)
                    ppo.sum_gradients_update(bs, ba, br, stepsize)
