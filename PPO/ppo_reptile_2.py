...
import argparse
ITR=10  # number of tasks
EP_MAX = 1000  # for single task, generally 2000 steps for 2 joints and 5000 steps for 3 joints have a good performance
EP_LEN = 20
GAMMA = 0.9
A_LR = 1e-4
C_LR = 2e-4
...
if args.test:
    ...
    plt.plot(np.arange(len(all_ep_r)), all_ep_r)
    print(np.arange(len(all_ep_r)), all_ep_r)
    plt.savefig('./ppo_reptile_test.png')