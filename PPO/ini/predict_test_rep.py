...
rand_v=tf.placeholder(tf.float32, [None, num_output])
...
with tf.variable_scope('pi'):
    trainable=True
    A_DIM=num_output
    l1 = tf.layers.dense(xs, 100, tf.nn.relu, trainable=trainable)
    ...
    mu =  40.*tf.layers.dense(l1, A_DIM, tf.nn.tanh, trainable=trainable)
...
if args.train:
    ...
    for i in range(train_epoches):
        ...
        for j in range (train_rep):
            v=(np.random.normal(size=(len(action_set),len(action_set[0])))).tolist()
            print('v: ',v[0])
            if i <0.3*train_epoches:
                pre,_, train_loss=sess.run([prediction,train_step,loss], feed_dict={xs: state_set, ys:action_set, rand_v: v, \
        ...
        v=(np.random.normal(size=(len(dev_set_a),len(dev_set_a[0])))).tolist()
        dev_loss=sess.run(loss, feed_dict={xs: dev_set_s, ys:dev_set_a,rand_v: v, \
            keep_prob: 0.9, lr:1e-5, phase_train.name: False})

        # v=(np.random.normal(size=(2000,3))).tolist()
        # dev_train_loss=sess.run(loss, feed_dict={xs: dev_training_set_s, ys:dev_training_set_a,rand_v: v, \
        #     keep_prob: 0.9, lr:1e-5, phase_train.name: False})
        print(i, train_loss)
        # print(pre)
...
if args.test:
    ...
    for i in range (num_test_steps):
        ...
        print(state)
        v=(np.random.normal(size=(1,num_output)).tolist())
        action=sess.run(prediction, feed_dict={xs: [state], rand_v:v, keep_prob: 1, lr:0.00001, phase_train.name: False})
