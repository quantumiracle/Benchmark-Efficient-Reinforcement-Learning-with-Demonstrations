...
with tf.variable_scope('pi'):
    trainable=True
    A_DIM=num_output
    l1 = tf.layers.dense(xs, 100, tf.nn.relu, trainable=trainable, kernel_initializer=tf.contrib.layers.xavier_initializer())
    ...
    # sigma = tf.layers.dense(l1, A_DIM, tf.nn.softplus, trainable=False, kernel_initializer=tf.contrib.layers.xavier_initializer()) # softplus to make it positive
    sigma = tf.constant(A_DIM*[0.1], dtype=tf.float32)
...
if args.train:
    ...
    for i in range(train_epoches):
        ...
        for j in range (train_rep):
            if i <0.5*train_epoches:
                s,pre,_, train_loss=sess.run([sigma, prediction,train_step,loss], feed_dict={xs: state_set, ys:action_set, \
