    ...
    def __init__(self, *, policy, ob_space, ac_space, nbatch_act, nbatch_train,
        ...
        #    saver.restore(sess, load_path)
            ...
            try:
                ...
                actor_var_list=tf.contrib.framework.get_variables('ppo2_model/pi')
...
def learn(*, save_path, network, env, total_timesteps, eval_env = None, seed=None, nsteps=2048, ent_coef=0.0, lr=3e-4,
    ...
    **network_kwargs:                 keyword arguments to the policy / network builder. See baselines.common/policies.py/build_policy and arguments to a particular type of network
    ...
    # Get the nb of env
    # nenvs = env.num_envs
    nenvs=1 # 1 for env: reacher 

    # Get state_space and action_space
    ob_space = env.observation_space
    ac_space = env.action_space
    # print(len(ac_space.shape))
    ...
    for update in range(1, nupdates+1):
        ...
        step_set.append(update)
        # print(returns)
        return_set.append(np.average(returns))
        plt.plot(step_set,return_set)
        plt.xlabel('Steps')
        plt.ylabel('Mean Episode Reward')
        plt.savefig('ppo2.png')
...
def testing(*, save_path, network, env, total_timesteps, eval_env = None, seed=None, nsteps=2048, ent_coef=0.0, lr=3e-4,
            vf_coef=0.5,  max_grad_norm=0.5, gamma=0.99, lam=0.95,
            log_interval=10, nminibatches=4, noptepochs=4, cliprange=0.2,
            save_interval=0, load_path=None, **network_kwargs):
    '''
...
def retraining(*, save_path, network, env, total_timesteps, eval_env = None, seed=None, nsteps=2048, ent_coef=0.0, lr=3e-4,
            vf_coef=0.5,  max_grad_norm=0.5, gamma=0.99, lam=0.95,
            log_interval=10, nminibatches=4, noptepochs=4, cliprange=0.2,
            save_interval=0, load_path=None, **network_kwargs):
    '''
    ...
    **network_kwargs:                 keyword arguments to the policy / network builder. See baselines.common/policies.py/build_policy and arguments to a particular type of network
    ...
    '''general restore to retrain'''
    # load_path=save_path
    # if load_path is not None:
    #     model.load(load_path)
    '''restore with initializing policy'''
    load_path='./ini/model/ppo2/ppo2'
    model.load_ini(load_path)
