...
save_file='./model/test/ddpg'
# save_file='./model/ppo2'
data_file0 = open("data8.p","rb")
...
def rescale_action(train_set, scale=None):
    max_action=np.max(np.abs(train_set))
    # print(max_action)
    if max_action>scale:
        return (scale/max_action)*np.array(train_set)
    else:
...
if args.train:
    ...
    '''preload the whole training set and sample to have dev_training_set'''
    for i in range (dev_bat+train_batches):
        print(i)
        try:
            data_set = np.array(pickle.load(data_file0))
        except EOFError:
            break
        # print(data_set)
        if i>=dev_bat:
            ...
            dev_training_set_a.append(rescale_action(data_set[sampling][1],1))
    print('preload succeed! size:',np.array(dev_training_set_s).shape)
    data_file0.close()
    ''' reload the same set during training'''
    data_file = open("data8.p","rb")

    for k in range(dev_bat):
        dev_set=np.array(pickle.load(data_file))
        # print(dev_set)
        [row_d, col_d]=dev_set.shape
        for j in range(row_d):
            dev_set_s.append(dev_set[j][0])
            # dev_set_a.append(screen_size*rescale_action(dev_set[j][1],1))
            dev_set_a.append(rescale_action(dev_set[j][1],1))
    ...
    for i in range(train_batches):
        ...
        for k in range(num_bat):
            ...
            for j in range (row):
                ...
                rescaled_action_set=rescale_action(train_set[j][1],1)
                '''action needs to multiply screen_size!'''
                # action_set.append(screen_size*rescaled_action_set)  # range +-100
                action_set.append(rescaled_action_set)
                # if j==1: # reduce size of dev_train_set, select 1 from each batch; 
                #     dev_training_set_s.append(train_set[j][0])
                #     dev_training_set_a.append(screen_size*rescaled_action_set)
            # print(train_set,action_set)
        ...
        for j in range (train_rep):
            if i <3500:
                ...
                pre,_, train_loss=sess.run([prediction,train_step,loss], feed_dict={xs: state_set, ys:action_set, \
                keep_prob: 0.9, lr:1e-6, phase_train.name: True})
...
if args.test:
    ...
    for i in range (num_test_steps):
        ...
        '''if multiply screen_size at training process, no need to do in step!'''
