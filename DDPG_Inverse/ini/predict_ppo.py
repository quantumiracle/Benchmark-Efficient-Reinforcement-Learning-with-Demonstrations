...
with tf.variable_scope('ppo2_model/pi'):
    # x=network_fn(xs)
    ...
    for i in range(1500):
        ...
        for k in range(num_bat):
            ...
            for j in range (row):
                ...
                action_set.append(1000*np.array(train_set[j][1]))
        ...
        if i <500:
            ...
            pre,_, train_loss=sess.run([prediction,train_step,loss], feed_dict={xs: state_set, ys:action_set, \
            keep_prob: 0.9, lr:1e-7, phase_train.name: True})
...
if args.test:
    ...
    for i in range (num_test_steps):
        ...
        time.sleep(0.2)
        print(i,action[0], screen_size*action[0])
