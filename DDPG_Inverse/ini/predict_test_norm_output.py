...
if args.train:
    ...
    for i in range (dev_bat+train_batches):
        ...
        if i>=dev_bat:
            '''no need for dev-train set for epoch training, dev-train is only different from train for episode training'''
            ...
            for j in range (row):
                ...
                rescaled_action_set=np.array(rescale_action(data_set[j][1],360))/360
            ...
            for j in range(row):
                ...
                dev_set_a.append(rescale_action(data_set[j][1],360)/360)
                # print(dev_set_a)
...
if args.test:
    ...
    for i in range (num_test_steps):
        ...
        pose=reacher.step(360*action[0])
