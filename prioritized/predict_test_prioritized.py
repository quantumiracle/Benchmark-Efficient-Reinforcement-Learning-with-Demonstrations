...
data_file = open("data_ini2goal.p","rb")
...
train_batches=2000
# train_epoches=2000
train_epoches=50  # supoptimal training  80 epoches
dev_loss_set=[]
if args.train:
    ...
    for i in range (dev_bat+train_batches):
        ...
        if i>=dev_bat:
            for j in range (row):
                batch_state_set.append(data_set[j][0])
                rescaled_action_set=rescale_action(data_set[j][1],360)
                batch_action_set.append(rescaled_action_set)
            state_set.append(batch_state_set)
            action_set.append(batch_action_set) 
        else:
            for j in range(row):
    ...
    for i in range(train_epoches):
        ...
        for j in range(train_batches):
            ...
            rep = 0
            '''
             if current batch_train_loss is larger than last epoch average train loss, 
            ...
            # while rep < 1:
                ...
                if i <0.3*train_epoches:
                    pre,_, train_loss=sess.run([prediction,train_step,loss], feed_dict={xs: state_set[j], ys:action_set[j], \
                    keep_prob: 0.9, lr:lr0, phase_train.name: True})
                    # print(pre)
                elif i< 0.6*train_epoches:
                    pre,_, train_loss=sess.run([prediction,train_step,loss], feed_dict={xs: state_set[j], ys:action_set[j], \
                    keep_prob: 0.9, lr:lr1, phase_train.name: True})
                else:
                    pre,_, train_loss=sess.run([prediction,train_step,loss], feed_dict={xs: state_set[j], ys:action_set[j], \
                    keep_prob: 0.9, lr:lr2, phase_train.name: True})
                print(j,train_loss, rep)
        ...
        train_loss_epoch_mean = np.average(train_loss_epoch)
        print('Epoch: {},  Mean Epoch Loss: {}'.format(i, train_loss_epoch_mean))

        loss_set.append(train_loss_epoch_mean)
        dev_loss_set.append(dev_loss)
        step_set.append(i)
    # current loss
    plt.plot(step_set,loss_set,'b', label='Train Loss',alpha=0.7)
    # loss on a fixed test set
    plt.plot(step_set,dev_loss_set,'r',label='Test Loss',alpha=0.7)
