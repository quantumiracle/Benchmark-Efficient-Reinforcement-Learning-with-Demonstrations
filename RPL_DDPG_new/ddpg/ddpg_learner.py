...
class DDPG(object):
    def __init__(self, actor, critic, memory, observation_shape, action_shape, param_noise=None, action_noise=None,
        ...
        batch_size=128, observation_range=(-1000., 1000.), action_range=(-360., 360.), return_range=(-np.inf, np.inf),
        ...
        self.normalized_critic_with_actor_tf = critic(normalized_obs0, self.res_actor_tf, reuse=True)
        self.critic_with_actor_tf = denormalize(tf.clip_by_value(self.normalized_critic_with_actor_tf, self.return_range[0], self.return_range[1]), self.ret_rms)
        _, res_target_actor_action = target_actor(normalized_obs1)
        Q_obs1 = denormalize(target_critic(normalized_obs1, res_target_actor_action), self.ret_rms)
    ...
    def setup_param_noise(self, normalized_obs0):
        ...
        self.perturbed_actor_tf, self.perturbed_res_actor_tf = param_noise_actor(normalized_obs0)
        ...
        adaptive_actor_tf, adaptive_res_actor_tf = adaptive_param_noise_actor(normalized_obs0)
        self.perturb_adaptive_policy_ops = get_perturbed_actor_updates(self.actor, adaptive_param_noise_actor, self.param_noise_stddev)
        self.adaptive_policy_distance = tf.sqrt(tf.reduce_mean(tf.square(self.res_actor_tf - adaptive_res_actor_tf)))
    ...
    def step(self, obs, apply_noise=True, compute_Q=True):
        ...
        if compute_Q:
            action, action_res, q = self.sess.run([self.actor_tf, res_actor_tf, self.critic_with_actor_tf], feed_dict=feed_dict)
        else:
            action, action_res = self.sess.run([self.actor_tf, res_actor_tf], feed_dict=feed_dict)
            q = None
    ...
    def load_ini(self,sess, load_path):
        ...
        #    print('actor_var:',actor_var_list)
           try:
                ...
                actor_saver.restore(self.sess, './ini/model/ddpg/ddpg')
    ...
    '''this part of code not working, because name 'target_actor' is different from 'actor', cannot 
    load initialization policy for target_actor directly'''
        #    actor_var_list1=tf.contrib.framework.get_variables('target_actor')
        #    print('actor_var:',actor_var_list1)
        #    try:
        #         actor_saver1=tf.train.Saver(actor_var_list1)
        #         actor_saver1.restore(self.sess, './ini/model/ddpg')
        #         print('Actor Load Succeed!')
        #    except:
        #         print('Actor Load Failed!')
